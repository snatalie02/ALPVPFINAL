...
export class DevController {
  ...
  static async simulateSkipDay(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await prismaClient.friends.updateMany({
        data: {
          time_stamp: twoDaysAgo,
          has_set_streak_today: false
        }
      })

      res.status(200).json({
        message: "⏭️ Simulated skip 1+ day. Streak should reset on next check-in."
      })
    } catch (error) {
      next(error)
    }
